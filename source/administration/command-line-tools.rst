...
~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  .. note::
    Users deactivated via this CLI command can continue to use Mattermost, if they are already logged in, until the user cache is manually refreshed.  Users who are deactivated and not currently logged in are prevented from logging in to Mattermost. If you want to immediately terminate a deactivated user's session, purge all caches in **System Console > Web Server > Purge All Caches** after running this command, or `use the API <https://api.mattermost.com/#tag/users%2Fpaths%2F~1users~1%7Buser_id%7D%2Fdelete>`_ to deactivate a user account which immediately terminates the session.
